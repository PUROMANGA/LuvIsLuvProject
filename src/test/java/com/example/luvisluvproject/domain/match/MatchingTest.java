...
public class MatchingTest {
	...
	public void setUp() {
		...
		Member me = testFactory.loginTestOf("송진영", "songjinyong@email.com", "test1234");
		Tag tag = testFactory.tagTestOf("게이", TagCategory.GENDER_IDENTITY);
		MemberTag memberTag = testFactory.memberTagTestOf(me, tag);

		memberRepository.save(me);

		for (int i = 0; i < 10; i++) {
			Member member = testFactory.loginTestOf("송진영" + i, "songjinyong@email.com" + i, "test1234");

			memberRepository.save(member);

			MemberTag memberTag1 = testFactory.memberTagTestOf(me, tag);
			MemberTag memberTag2 = testFactory.memberTagTestOf(member, tag);
	...
	// 		String tagName = memberTag.getTagId().getName();
